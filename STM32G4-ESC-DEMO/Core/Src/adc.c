...
void MX_ADC1_Init(void)
{
  ...
  */
  ...
  GPIO_InitStruct.Mode = LL_GPIO_MODE_ANALOG;
  GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;
  LL_GPIO_Init(BEMF2_GPIO_Port, &GPIO_InitStruct);

  GPIO_InitStruct.Pin = BEMF3_Pin;
  GPIO_InitStruct.Mode = LL_GPIO_MODE_ANALOG;
  GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;
  LL_GPIO_Init(BEMF3_GPIO_Port, &GPIO_InitStruct);
...
uint16_t adc_read_blocking(void) {
	...
	uint16_t adc_read = LL_ADC_REG_ReadConversionData12(ADC1);
	return adc_read;
//
//	// wait for EOS then clear
